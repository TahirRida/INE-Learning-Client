...
const ProfilePage = () => {
  ...
  const usernameClicked = location.state && location.state.usernameClicked ? location.state.usernameClicked : '';
  const userClickedOwnedCourses = location.state && location.state.userClickedOwnedCourses ? location.state.userClickedOwnedCourses : [];
  ...
  const [openUpdatedProfileSnackbar, setOpenUpdatedProfileSnackbar] = useState(false);
  ...
  useEffect(() => {
    setIsCurrentUserProfile(usernameClicked ? usernameStored === usernameClicked : usernameStored);
  }, [usernameClicked]);
  ...
  const handleUpdate = async () => {
    try {
      ...
      const updatedCourse = await axios.put(`${baseURL}/courses/${selectedCourseId}`, {
        ...updatedValues,
  ...
  const handleConfirmation = async (confirmed) => {
    if (confirmed) {
      try {
        ...
        const { username: updatedUsername, password: updatedPassword } = response.data;
    ...
    <div className="profile-containerAll">
      ...
      <div className="profile-container">
        ...
        <div className="profile-info">
          ...
          <h2>{isCurrentUserProfile ? `Welcome ${usernameClicked ? usernameClicked : usernameStored}` : `${usernameClicked}'s Profile`}</h2>
          <>
      {isCurrentUserProfile && (
        <>
          <button
          ...
          <Dialog open={openPopDialog} onClose={() => handleConfirmation(false)}>
        <DialogTitle>Confirm Profile Update</DialogTitle>
        <DialogContent>
          <DialogContentText>
            Are you sure you want to update your profile? You will be redirected to the Login Page to re-sign in.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
        ...
        <h3>{isCurrentUserProfile ? "Here are your posted videos" : `Here are ${usernameClicked}'s posts`}</h3>
        {isCurrentUserProfile ? (
          <div className="course-list">
            {userOwnedCourses.map(course => (
              <div key={course.courseId} className="course-card">
                <CourseCard course={course} />
                <Button className="update-button" onClick={() => handleClickOpen(course.courseId)}>
